...
describe('blog lists', () => {
  it('supports valid mixed nesting in either direction', () => {
    ...
    expect(
      container.querySelector('ul.rvds-bulleted-list > li > ol.rvds-numbered-list'),
    ).not.toBeNull();
    expect(
      container.querySelector('ol.rvds-numbered-list > li > ul.rvds-bulleted-list'),
    ).not.toBeNull();
  });

  it('preserves native ordered-list controls and item values', () => {
    const { container } = render(
      <NumberedList reversed start={5}>
        <li value={8}>Eight</li>
        <li>Seven</li>
      </NumberedList>,
    );
