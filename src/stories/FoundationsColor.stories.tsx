...
function ColorTokens() {
  ...
  return <Stack><Heading>Semantic color</Heading><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(9rem,1fr))', gap: '1rem' }}>{colors.map((name) => <div key={name}><div style={{ background: `var(--rvds-color-${name})`, border: '1px solid var(--rvds-color-border)', height: '4rem' }} /><code>--rvds-color-{name}</code></div>)}</div></Stack>;
}
